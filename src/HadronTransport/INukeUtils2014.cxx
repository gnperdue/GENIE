...
#include "HadronTransport/INukeUtils2014.h"
...
//____________________________________________________________________________
double genie::utils::intranuke2014::MeanFreePath(
...
//
  ...
  ke = TMath::Max(INukeHadroData2014::fMinKinEnergy,   ke);
  ke = TMath::Min(INukeHadroData2014::fMaxKinEnergyHN, ke);

  // get total xsection for the incident hadron at its current
  // kinetic energy
  double sigtot = 0;
  double ppcnt = (double) Z/ (double) A; // % of protons remaining
  INukeHadroData2014 * fHadroData2014 = INukeHadroData2014::Instance();

  if (pdgc == kPdgPiP)
    { sigtot = fHadroData2014 -> XSecPipp_Tot() -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2014 -> XSecPipn_Tot() -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgPi0)
    { sigtot = fHadroData2014 -> XSecPi0p_Tot() -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2014 -> XSecPi0n_Tot() -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgPiM)
    { sigtot = fHadroData2014 -> XSecPipn_Tot() -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2014 -> XSecPipp_Tot() -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgProton)
    { sigtot = fHadroData2014 -> XSecPp_Tot()   -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2014 -> XSecPn_Tot()   -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgNeutron)
    { sigtot = fHadroData2014 -> XSecPn_Tot()   -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2014 -> XSecNn_Tot()   -> Evaluate(ke)*(1-ppcnt);}
  else if (pdgc == kPdgKP)
    { sigtot = fHadroData2014 -> XSecKpN_Tot()  -> Evaluate(ke);
      sigtot*=1.2;}
  else if (pdgc == kPdgGamma)
    { sigtot = fHadroData2014 -> XSecGamp_fs()  -> Evaluate(ke)*ppcnt;
      sigtot+= fHadroData2014 -> XSecGamn_fs()  -> Evaluate(ke)*(1-ppcnt);}
  else {
     return 0;
  }
...
//____________________________________________________________________________
double genie::utils::intranuke2014::MeanFreePath_Delta(
...
//____________________________________________________________________________
double genie::utils::intranuke2014::ProbSurvival(
...
//  R0: R0 in R=R0*A^1/3 (units:fm). Def. 1.4
   ...
   while(1) {
     ...
     double mfp = 
       genie::utils::intranuke2014::MeanFreePath(pdgc,x4_curr,p4,A,Z,nRpi,nRnuc);
...
//____________________________________________________________________________
double genie::utils::intranuke2014::Dist2Exit(
...
//____________________________________________________________________________
double genie::utils::intranuke2014::Dist2ExitMFP(
   int pdgc, const TLorentzVector & x4, const TLorentzVector & p4, 
...
// 
   ...
   while(1) {
        ...
        double lambda = genie::utils::intranuke2014::MeanFreePath(pdgc,x4_curr,p4,A,Z);
        d_mfp += (step/lambda);
...
//____________________________________________________________________________
void genie::utils::intranuke2014::StepParticle(
...
//    and equilibrium
//    Alex Bell -- 6/17/2008
void genie::utils::intranuke2014::PreEquilibrium(
  ...
  bool DoFermi, double FermiFac, const NuclearModelI* Nuclmodel, double NucRmvE, EINukeMode mode)
{
  ...
  bool success = genie::utils::intranuke2014::PhaseSpaceDecay(ev,p,list,RemnP4,NucRmvE,mode);
...
*/
  ...
  genie::utils::intranuke2014::Equilibrium(ev,t,RemnA,RemnZ,RemnP4,DoFermi,FermiFac,Nuclmodel,NucRmvE,mode);
...
// Alex Bell -- 6/17/2008
void genie::utils::intranuke2014::Equilibrium(
  GHepRecord * ev, GHepParticle * p, int &RemnA, int &RemnZ, TLorentzVector &RemnP4,
  bool DoFermi, double FermiFac, const NuclearModelI* Nuclmodel, double NucRmvE, EINukeMode mode)
{
...
//___________________________________________________________________________
bool genie::utils::intranuke2014::TwoBodyCollision(
...
//___________________________________________________________________________
bool genie::utils::intranuke2014::TwoBodyKinematics(
...
//___________________________________________________________________________
bool genie::utils::intranuke2014::ThreeBodyKinematics(
  ...
  bool DoFermi, double FermiFac, double FermiMomentum, const NuclearModelI* Nuclmodel)
{
  ...
  // Aaron Meyer (7/15/10)
  //
  // Kinematics used in utils::intranuke2014::PionProduction
...
//___________________________________________________________________________
bool genie::utils::intranuke2014::PionProduction(
  ...
  //
   ...
   if ((p1code==kPdgPi0)||(p1code==kPdgPiP)||(p1code==kPdgPiM)) {
   ...
   } else if(p1code==kPdgProton||p1code==kPdgNeutron) //nucleon probes
    {
      ...
      double etapp2ppPi0 = 
	utils::intranuke2014::CalculateEta(pMass,tote,pMass,pMass+pMass,pLib->Find(111)->Mass());
      double etapp2pnPip = 
	utils::intranuke2014::CalculateEta(pLib->Find(p1code)->Mass(),tote,((p1code==kPdgProton)?pMass:nMass),
				       pMass+nMass,pLib->Find(211)->Mass());
      double etapn2nnPip = 
	utils::intranuke2014::CalculateEta(pMass,tote,nMass,nMass+nMass,pLib->Find(211)->Mass());
      double etapn2ppPim = 
	utils::intranuke2014::CalculateEta(pMass,tote,nMass,pMass+pMass,pLib->Find(211)->Mass());
   ...
   if(genie::utils::intranuke2014::ThreeBodyKinematics(
...
//___________________________________________________________________________
double genie::utils::intranuke2014::CalculateEta(double Minc, double nrg, double Mtarg,
			       double Mtwopart, double Mpi)
{
  ...
  //For pion cross sections, in utils::intranuke2014::PionProduction
...
//___________________________________________________________________________
bool genie::utils::intranuke2014::PhaseSpaceDecay(
